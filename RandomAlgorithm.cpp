...
Result RandomAlgorithm::greedyAlgorithm(std::vector<Yard> yards, std::list<Segment> segments, std::list<Segment> segments_in_yard, Date today)
{
	...
	while (!segments.empty() || !segments_to_enter.empty()) {
		...
		for (auto it = segments_to_enter.begin(); it != segments_to_enter.end(); /* 注意：不自动递增 */) {
			...
			// 先找出可行位置：用左下角格子坐标表示
			std::vector<Coordinate> valid_coordinates;
			// 遍历堆场找出所有可行位置。由于yards已按距离升序排序，故valid_coordinates中前面的就是距离短的
			for (int n = 0; n < yards.size(); ++n) {
				Yard y = yards[n];
				// 遍历堆场每格
				for (int i = 0; i < y.free_map.size(); ++i) {
			...
			if (valid_coordinates.empty()) {
				...
				// 从valid_coordinates中，用greedy法选一个位置放分段。
				// greedy：每次选择了到总段最近的堆场，在堆场中的位置是左下到右上第一个可以放的
				s.coordinate = valid_coordinates[0];
